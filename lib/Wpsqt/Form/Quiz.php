...
class Wpsqt_Form_Quiz extends Wpsqt_Form {
	...
	public function __construct( array $options = array() ){
		...
		$this->addOption("wpsqt_name", "Name", "text", $options['name'], "What you would like the quiz to be called." )
			 ...
			 ->addOptionA("wpsqt_finish_display", "Finish Display",'select', $options['finish_display'], "What should be displayed on the finishing of the quiz.", array("Finish message","Quiz Review","Both"))
			 ...
			 ->addOptionA("wpsqt_pdf_template", "PDF Template", "textarea", $options['pdf_template'], "The template for the PDF. <strong>If you're not using pdf certificates then leave blank.</strong> <a href=\"#template_tokens\">Click Here</a> to see the tokens that can be used.", array(), false)
			 ->addOptionA("wpsqt_use_pdf", "PDF Certificates","yesno", $options['use_pdf'], "Allow the user to download a PDF certificate.")
			 ...
			 ->addOptionA("wpsqt_fail_review", "Quiz review on fail", "yesno", $options['fail_review'], "Do you want to show quiz review page if the taker fails?");
